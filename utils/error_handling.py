        ...
        try:
            ...
            if len(args) >= 2 and hasattr(args[0], 'effective_chat') and hasattr(args[1], 'bot'):
                try:
                    ...
                    if config.delete_user_commands and hasattr(update, 'effective_message') and getattr(update.effective_message, 'message_id', None):
                        schedule_message_deletion(
                            chat_id=update.effective_chat.id,
...
class CircuitBreakerManager:
    ...
    def _cleanup_inactive_breakers(self):
        ...
        for name, breaker in self.circuit_breakers.items():
            ...
            if (now - breaker.last_failure_time > 86400 and 
                breaker.state == 'CLOSED' and 
...
def with_rate_limit(name: Optional[str] = None, max_calls: int = 10, time_window: int = 60):
    """速率限制装饰器"""
    def decorator(func):
...
class ErrorAnalyzer:
    ...
    def analyze_http_error(error: Exception) -> dict:
        ...
        if isinstance(error, httpx.TimeoutException):
            error_info.update({
                'type': 'timeout',
                'user_message': "⏱️ 请求超时，请稍后重试。"
            })
        elif isinstance(error, httpx.ConnectError):
            error_info.update({
                'type': 'connection',
                'user_message': "🌐 网络连接失败，请检查网络状态。"
            })
