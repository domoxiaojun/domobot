...
class BotConfig:
    ...
    # 定时清理配置
    spotify_weekly_cleanup: bool = False
    disney_weekly_cleanup: bool = False
    ...
    # 国家代码配置
    default_countries: Dict[str, list] = field(default_factory=lambda: {
...
class ConfigManager:
    ...
    def _load_from_environment(self):
        ...
        # 消息自动删除配置
        self.config.auto_delete_delay = int(os.getenv('AUTO_DELETE_DELAY', '180'))
        ...
        if cache_group_ids_str:
            self.config.user_cache_group_ids = [int(gid.strip()) for gid in cache_group_ids_str.split(',') if gid.strip()]
