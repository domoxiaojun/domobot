...
class DisneyPriceBot(PriceQueryService):
    ...
    async def _format_price_message(self, country_code: str, price_info: dict) -> str | None:
        ...
        country_name_cn = price_info.get('name_cn', SUPPORTED_COUNTRIES.get(country_code.upper(), {}).get('name', country_code))
    ...
    async def get_top_cheapest(self, top_n: int = 10) -> str:
        ...
        for code, country_data in self.data.items():
            ...
            if comparison_price is not None:
                country_name_cn = country_data.get('name_cn', SUPPORTED_COUNTRIES.get(code.upper(), {}).get('name', code))
                
                premium_plan_details = next((plan for plan in country_data.get('plans', []) if ('Premium' in plan.get('plan_name', '') or '高級版' in plan.get('plan_name', ''))), None)
        ...
        for idx, country in enumerate(top_countries, 1):
            ...
            if plan_details:
                ...
                message_lines.append(
                    f"{rank_emoji} {country['name_cn']} ({country_code_upper}) {country_flag}"
                )
...
def set_rate_converter(converter: RateConverter):
    """Initializes the DisneyPriceBot service."""
...
command_factory.register_command("ds_cleancache", disney_plus_clean_cache_command, permission=Permission.ADMIN, description="清理Disney+缓存")