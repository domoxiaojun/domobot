...
from utils.country_data import get_country_flag, SUPPORTED_COUNTRIES
...
class NetflixPriceBot(PriceQueryService):
    ...
    async def _format_price_message(self, country_code: str, price_info: Dict) -> str:
        ...
        for i, key in enumerate(available_plans):
...
def set_rate_converter(converter: RateConverter):
    """Initializes the NetflixPriceBot service."""
...
command_factory.register_command("nf_cleancache", netflix_clean_cache_command, permission=Permission.ADMIN, description="清理Netflix缓存")