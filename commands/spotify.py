...
from utils.country_data import get_country_flag, SUPPORTED_COUNTRIES, COUNTRY_NAME_TO_CODE
...
COUNTRY_CODES_CN = {
    # Africa
    "AO": "安哥拉", "BJ": "贝宁", "BW": "博茨瓦纳", "BF": "布基纳法索", "BI": "布隆迪",
    "CV": "佛得角", "CM": "喀麦隆", "TD": "乍得", "KM": "科摩罗", "CI": "科特迪瓦",
    "CD": "刚果民主共和国", "DJ": "吉布提", "EG": "埃及", "GQ": "赤道几内亚",
    "SZ": "斯威士兰", "ET": "埃塞俄比亚", "GA": "加蓬", "GM": "冈比亚", "GH": "加纳",
    "GN": "几内亚", "GW": "几内亚比绍", "KE": "肯尼亚", "LS": "莱索托", "LR": "利比里亚",
    "LY": "利比亚", "MG": "马达加斯加", "MW": "马拉维", "ML": "马里", "MR": "毛里塔尼亚",
    "MU": "毛里求斯", "MA": "摩洛哥", "MZ": "莫桑比克", "NA": "纳米比亚", "NE": "尼日尔",
    "NG": "尼日利亚", "CG": "刚果共和国", "RW": "卢旺达", "ST": "圣多美和普林西比",
    "SN": "塞内加尔", "SC": "塞舌尔", "SL": "塞拉利昂", "ZA": "南非", "TZ": "坦桑尼亚",
    "TG": "多哥", "TN": "突尼斯", "UG": "乌干达", "ZM": "赞比亚", "ZW": "津巴布韦",
    
    # Asia
    "AM": "亚美尼亚", "AZ": "阿塞拜疆", "BH": "巴林", "BD": "孟加拉国", "BT": "不丹",
    "BN": "文莱", "KH": "柬埔寨", "CY": "塞浦路斯", "GE": "格鲁吉亚", "HK": "香港",
    "IN": "印度", "ID": "印度尼西亚", "IQ": "伊拉克", "IL": "以色列", "JP": "日本", "JO": "约旦",
    "KZ": "哈萨克斯坦", "KW": "科威特", "KG": "吉尔吉斯斯坦", "LA": "老挝", "LB": "黎巴嫩",
    "MO": "澳门", "MY": "马来西亚", "MV": "马尔代夫", "MN": "蒙古", "NP": "尼泊尔",
    "OM": "阿曼", "PK": "巴基斯坦", "PS": "巴勒斯坦", "PH": "菲律宾", "QA": "卡塔尔",
    "SA": "沙特阿拉伯", "SG": "新加坡", "KR": "韩国", "LK": "斯里兰卡", "TW": "台湾",
    "TJ": "塔吉克斯坦", "TH": "泰国", "TL": "东帝汶", "TR": "土耳其", "AE": "阿联酋",
    "UZ": "乌兹别克斯坦", "VN": "越南",
    
    # Europe
    "AL": "阿尔巴尼亚", "AD": "安道尔", "AT": "奥地利", "BY": "白俄罗斯", "BE": "比利时",
    "BA": "波黑", "BG": "保加利亚", "HR": "克罗地亚", "CZ": "捷克",
    "DK": "丹麦", "EE": "爱沙尼亚", "FI": "芬兰", "FR": "法国", "DE": "德国",
    "GR": "希腊", "HU": "匈牙利", "IS": "冰岛", "IE": "爱尔兰", "IT": "意大利",
    "XK": "科索沃", "LV": "拉脱维亚", "LI": "列支敦士登", "LT": "立陶宛", "LU": "卢森堡",
    "MT": "马耳他", "MD": "摩尔多瓦", "MC": "摩纳哥", "ME": "黑山", "NL": "荷兰",
    "MK": "北马其顿", "NO": "挪威", "PL": "波兰", "PT": "葡萄牙", "RO": "罗马尼亚",
    "SM": "圣马力诺", "RS": "塞尔维亚", "SK": "斯洛伐克", "SI": "斯洛文尼亚", "ES": "西班牙",
    "SE": "瑞典", "CH": "瑞士", "UA": "乌克兰", "GB": "英国",
    
    # Latin America and the Caribbean
    "AG": "安提瓜和巴布达", "AR": "阿根廷", "BS": "巴哈马", "BB": "巴巴多斯",
    "BZ": "伯利兹", "BO": "玻利维亚", "BR": "巴西", "CL": "智利", "CO": "哥伦比亚",
    "CR": "哥斯达黎加", "CW": "库拉索", "DM": "多米尼克", "DO": "多米尼加",
    "EC": "厄瓜多尔", "SV": "萨尔瓦多", "GD": "格林纳达", "GT": "危地马拉", "GY": "圭亚那",
    "HT": "海地", "HN": "洪都拉斯", "JM": "牙买加", "MX": "墨西哥", "NI": "尼加拉瓜",
    "PA": "巴拿马", "PY": "巴拉圭", "PE": "秘鲁", "KN": "圣基茨和尼维斯",
    "LC": "圣卢西亚", "VC": "圣文森特和格林纳丁斯", "SR": "苏里南",
    "TT": "特立尼达和多巴哥", "UY": "乌拉圭", "VE": "委内瑞拉",
    
    # Northern America
    "CA": "加拿大", "US": "美国",
    
    # Oceania
    "AU": "澳大利亚", "FJ": "斐济", "KI": "基里巴斯", "MH": "马绍尔群岛",
    "FM": "密克罗尼西亚", "NR": "瑙鲁", "NZ": "新西兰", "PW": "帕劳",
    "PG": "巴布亚新几内亚", "WS": "萨摩亚", "SB": "所罗门群岛", "TO": "汤加",
    "TV": "图瓦卢", "VU": "瓦努阿图"
}

class SpotifyPriceBot(PriceQueryService):
    PRICE_URL = "https://raw.githubusercontent.com/domoxiaojun/spotify-prices/refs/heads/main/spotify_prices_cny_sorted.json"
    ...
    async def _format_price_message(self, country_code: str, price_info: Dict) -> str:
        ...
        country_name_cn = (
            country_info.get('name_cn') or 
            COUNTRY_CODES_CN.get(country_code.upper()) or
            price_info.get('country_name_cn') or 
            COUNTRY_CODES.get(country_code.upper()) or
            price_info.get('country_name', country_code)
        )
    ...
    async def get_top_cheapest(self, top_n: int = 10) -> str:
        ...
        if top_10_data:
            ...
            for item in top_10_data[:top_n]:
                ...
                country_name_cn = (
                    item.get('country_name_cn') or
                    SUPPORTED_COUNTRIES.get(country_code, {}).get('name_cn') or
                    COUNTRY_CODES_CN.get(country_code) or
                    COUNTRY_CODES.get(country_code) or
                    item.get('country_name', country_code)
                )
                ...
                message_lines.append(
                    f"{rank_emoji} {country_name_cn} ({country_code}) {country_flag}"
                )
...
def set_dependencies(cm: CacheManager, rc: RateConverter):
...
async def spotify_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        ...
        try:
            ...
            logger.info(f"🔧 Scheduled deletion for Spotify messages - Bot: {bot_message_id} (after {config.auto_delete_delay}s), User: {user_command_id} (after {config.user_command_delete_delay}s)")
...
command_factory.register_command("sp_cleancache", spotify_clean_cache_command, permission=Permission.ADMIN, description="清理Spotify缓存")