...
def get_currency_symbol(currency_code: str) -> str:
    ...
    common_symbols = {
        ...
        "KRW": "₩", "INR": "₹", "RUB": "₽", "TRY": "₺", "THB": "฿",
        "IDR": "Rp", "MYR": "RM", "SGD": "S$",
        "CAD": "C$", "HKD": "HK$", "TWD": "NT$", "BRL": "R$",
        "NGN": "₦", "UAH": "₴", "ILS": "₪", "CZK": "Kč", "PLN": "zł",
        "SEK": "kr", "NOK": "kr", "DKK": "kr", "CHF": "CHF", "AED": "د.إ",
        "SAR": "ر.س", "QAR": "ر.ق", "KWD": "د.ك", "BHD": ".د.ب", "OMR": "ر.ع.",
        "EGP": "£", "MXN": "$", "ARS": "$", "CLP": "$", "COP": "$",
        "PEN": "S/", "VES": "Bs.",
        "NZD": "NZ$", "BGN": "лв",
        "HUF": "Ft", "ISK": "kr", "LKR": "Rs", "MNT": "₮", "KZT": "₸",
        "AZN": "₼", "AMD": "֏", "GEL": "₾", "MDL": "L", "RON": "lei",
        "RSD": "дин", "BYN": "Br", "UZS": "сўм", "LAK": "₭",
        "KHR": "៛", "MMK": "Ks", "BDT": "৳", "NPR": "₨", "PKR": "₨",
        "PHP": "₱", "VND": "₫", "LBP": "ل.ل", "JOD": "د.ا", "SYP": "£",
        "YER": "﷼", "DZD": "دج", "LYD": "ل.د", "MAD": "د.م.", "TND": "د.ت",
        "FJD": "$", "WST": "T", "TOP": "T$", "PGK": "K",
        "SBD": "$", "SHP": "£", "STD": "Db",
        "TJS": "ЅМ", "TMT": "m", "ZAR": "R",
        "ZWL": "$", "BYR": "Br", "GHS": "₵", "MOP": "MOP$", "UYU": "$U",
        "VEF": "Bs.F.", "XAF": "FCFA", "XCD": "$", "XOF": "CFA",
        "XPF": "₣"
    }
...
async def rate_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Validate currencies
    await rate_converter.get_rates() # Ensure rates are loaded
    if not rate_converter.rates:
        error_message = "❌ 无法获取汇率数据。请稍后重试。"
        await message.edit_text(
            foldable_text_v2(error_message),
            "MarkdownV2"
        )
...
async def rate_clean_cache_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        config = get_config()
        if rate_converter:
            rate_converter.cache_manager.clear_cache(key="exchange_rates")
            ...
            sent_message = await context.bot.send_message(
                chat_id=update.message.chat_id,
                text=foldable_text_v2(warning_message),
                parse_mode="MarkdownV2",
            )
            schedule_message_deletion(
                chat_id=sent_message.chat_id,
        ...
        error_message = f"❌ 清理汇率缓存时发生错误: {str(e)}"
        config = get_config()
        sent_message = await context.bot.send_message(
            chat_id=update.message.chat_id,
            text=foldable_text_v2(error_message),
...
command_factory.register_command("rate_cleancache", rate_clean_cache_command, permission=Permission.ADMIN, description="清理汇率缓存")